...
def between(x, y):
    return np.concatenate(
        [
            np.sqrt(np.sum(x * x, axis=-1) * np.sum(y * y, axis=-1))[..., None] + np.sum(x * y, axis=-1)[..., None],
            _fast_cross(x, y),
        ],
        axis=-1,
    )
...
def fk_vel(lrot, lpos, lvel, lang, parents):
    ...
    for i in range(1, len(parents)):
        ...
        gr.append(mul(gr[parents[i]], lrot[..., i : i + 1, :]))
        gv.append(
            mul_vec(gr[parents[i]], lvel[..., i : i + 1, :])
    ...
    if order == "zyx":
        ...
        return np.concatenate(
            [
                ...
                np.arctan2(2 * (q0 * q2 - q1 * q3), q0 * q0 - q1 * q1 - q2 * q2 + q3 * q3),
            ],
            axis=-1,
        )
        ...
        return np.concatenate(
            [
                ...
                np.arcsin((2 * (q0 * q1 - q2 * q3)).clip(-1, 1)),
                np.arctan2(2 * (q1 * q2 + q0 * q3), q0 * q0 - q1 * q1 + q2 * q2 - q3 * q3),
            ],
            axis=-1,
        )
...
def from_xform_xy(x):
    ...
    return from_xform(np.concatenate([c0[..., None], c1[..., None], c2[..., None]], axis=-1))
