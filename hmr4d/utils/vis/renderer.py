...
class Renderer:
    ...
    def export_mesh(self, verts, colors, faces=None):
        """
        :param verts (N, V, 3), potential multiple people
        :param colors (N, 3) or (N, V, 3)
        :param faces (N, F, 3), optional, otherwise self.faces is used will be used
        ...
        verts = verts.cpu().numpy().squeeze()
        faces = faces.cpu().numpy().squeeze()
        colors = colors.cpu().numpy().squeeze()
        
        # process=False to avoid creating a new mesh
        mesh = trimesh.Trimesh(
            verts, faces, vertex_colors=colors, process=False
        )
