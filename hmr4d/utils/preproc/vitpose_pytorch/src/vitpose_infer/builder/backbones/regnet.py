...
class RegNet(ResNet):
    ...
    def __init__(
        ...
        if isinstance(arch, str):
            ...
            raise TypeError('Expect "arch" to be either a string ' f"or a dict, got {type(arch)}")
        ...
        if self.deep_stem:
            raise NotImplementedError("deep_stem has not been implemented for RegNet")
    ...
    def _make_stem_layer(self, in_channels, base_channels):
        self.conv1 = build_conv_layer(
            self.conv_cfg, in_channels, base_channels, kernel_size=3, stride=2, padding=1, bias=False
        )
    ...
    @staticmethod
    def generate_regnet(initial_width, width_slope, width_parameter, depth, divisor=8):
