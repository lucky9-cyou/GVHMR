...
def draw_points(image, points, color_palette="tab20", palette_samples=16, confidence_threshold=0.5):
    ...
    try:
        ...
        colors = (
            np.round(np.array(plt.get_cmap(color_palette)(np.linspace(0, 1, palette_samples))) * 255)
            .astype(np.uint8)[:, -2::-1]
            .tolist()
        )
...
def draw_skeleton(
    image, points, skeleton, color_palette="Set2", palette_samples=8, person_index=0, confidence_threshold=0.5
):
    """
...
def save_images(
    images, target, joint_target, output, joint_output, joint_visibility, summary_writer=None, step=0, prefix=""
):
    """
