...
def pose_points_yolo5(detector, image, pose, tracker, tensorrt):
    ...
    timer_det.toc()
    logger.info("DET FPS -- %s", 1.0 / timer_det.average_time)
    # print(detections.shape)
    dets = detections.xyxy[0]
    dets = dets[dets[:, 5] == 0.0]
    ...
    # tracker.update()
    timer_track.toc()
    logger.info("TRACKING FPS --%s", 1.0 / timer_track.average_time)
    device = "cuda"
        ...
        if images.shape[0] > 0:
            ...
            if tensorrt:
                ...
                with torch.no_grad():
                    ...
                    timer_pose.toc()
                    logger.info("POSE FPS -- %s", 1.0 / timer_pose.average_time)
            else:
                with torch.no_grad():
            ...
            dim1 = torch.tensor(1.0 / 64, device=device)
            dim2 = torch.tensor(1.0 / 48, device=device)
